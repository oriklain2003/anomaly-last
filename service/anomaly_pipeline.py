...
class AnomalyPipeline:
    ...
    def analyze(self, flight, active_flights_context: Dict[str, Any] = None, repository=None) -> Dict[str, Any]:
        ...
        # --- Layer 2: XGBoost ---
        # Note: XGBoost uses aggregate features, cannot pinpoint specific anomaly locations
        t0 = time.time()
        if self.xgb_detector:
            try:
                res = self.xgb_detector.predict(flight_active)
                if "error" not in res:
                    # Add empty anomaly_points since XGBoost can't localize anomalies
                    res["anomaly_points"] = []
                    ...
                    results["layer_2_xgboost"] = {"error": res["error"], "anomaly_points": []}
            except Exception as e:
                    results["layer_2_xgboost"] = {"error": str(e), "anomaly_points": []}
        ...
        if self.deep_detector:
            try:
                ...
                if "error" not in res:
                    ...
                    results["layer_3_deep_dense"] = {"error": res["error"], "anomaly_points": res.get("anomaly_points", [])}
            except Exception as e:
                    results["layer_3_deep_dense"] = {"error": str(e), "anomaly_points": []}
        ...
        if self.cnn_detector:
            try:
                ...
                if "error" not in res:
                    ...
                    results["layer_4_deep_cnn"] = {"error": res["error"], "anomaly_points": res.get("anomaly_points", [])}
            except Exception as e:
                results["layer_4_deep_cnn"] = {"error": str(e), "anomaly_points": []}
        ...
        if self.trans_detector:
            try:
                ...
                if "error" not in res:
                    results["layer_5_transformer"] = res
                    if res.get("is_anomaly"):
                        is_anomaly_any = True
                        summary_triggers.append("Transformer")
                else:
                    results["layer_5_transformer"] = {"error": res["error"], "anomaly_points": []}
            except Exception as e:
                    results["layer_5_transformer"] = {"error": str(e), "anomaly_points": []}
        ...
        if hasattr(self, 'hybrid_detector') and self.hybrid_detector:
            try:
                ...
                if "error" not in res:
                    ...
                    results["layer_6_hybrid"] = {"error": res["error"], "anomaly_points": res.get("anomaly_points", [])}
            except Exception as e:
                results["layer_6_hybrid"] = {"error": str(e), "anomaly_points": []}
