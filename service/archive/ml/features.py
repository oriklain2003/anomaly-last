...
# Check if we are running as a script or module to handle imports correctly
try:
...
class FeatureExtractor:
    ...
    Focuses on dynamic features (rates, accelerations) AND rolling window history
    to capture sustained behaviors like holding patterns or continuous climbs.
    """
    ...
    def extract_flight_features(self, flight: FlightTrack) -> List[Dict[str, float]]:
        ...
        # 300s = 5 minutes (good for holding patterns)
        # 60s = 1 minute (good for sustained climb/descent)
        ...
        for idx, point in enumerate(points):
            ...
            # For turn, we sum the absolute changes to detect "activity" 
            # or signed changes for total rotation? 
            # Holding patterns usually circle in one direction, so signed sum is better 
            # to detect 360 degrees.
            ...
            roll_speed_300.update(point.timestamp, gspeed_curr) # Note: storing value, not delta
            avg_speed_300 = roll_speed_300.average()

            if point.track is not None:
                heading_sin = math.sin(math.radians(point.track))
                heading_cos = math.cos(math.radians(point.track))
            else:
                heading_sin = 0.0
                heading_cos = 0.0
