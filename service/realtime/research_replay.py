...
class ResearchReplay:
    def __init__(self, db_path: str):
        ...
        # Use the pipeline's DB path for the cache so rules can find data
        # If pipeline didn't find a DB, it defaults to rules/flight_tracks.db
        self.cache_db_path = getattr(self.pipeline, "db_path", Path("rules/flight_tracks.db"))
        
        self.setup_db()
        self.setup_cache_db()
        
    def setup_cache_db(self):
        """Ensure the cache/rules DB exists and has the tracks table."""
        self.cache_db_path.parent.mkdir(exist_ok=True, parents=True)
        try:
            conn = sqlite3.connect(str(self.cache_db_path))
            cursor = conn.cursor()
            
            # Matches FlightRepository schema
            cursor.execute("""
            ...
            conn.commit()
            conn.close()
            logger.info(f"Cache DB initialized at {self.cache_db_path}")
        except Exception as e:
            logger.error(f"Failed to setup cache DB: {e}")
    ...
    def update_cache(self, flight: FlightTrack):
        """Save flight to the shared cache DB for rule context (Proximity)."""
        try:
            conn = sqlite3.connect(str(self.cache_db_path))
            cursor = conn.cursor()
            ...
            logger.error(f"Failed to update cache for {flight.flight_id}: {e}")
    ...
    def cleanup_cache(self, retention_cutoff_ts: int):
        """Delete tracks older than the retention cutoff from the shared cache."""
        try:
            conn = sqlite3.connect(str(self.cache_db_path))
            cursor = conn.cursor()
            
            cursor.execute("DELETE FROM flight_tracks WHERE timestamp < ?", (retention_cutoff_ts,))
            deleted_count = cursor.rowcount
            
            conn.commit()
            conn.close()
            
            logger.info(f"Cache Cleanup: Deleted {deleted_count} points older than {datetime.fromtimestamp(retention_cutoff_ts)}")
        except Exception as e:
            logger.error(f"Cache cleanup failed: {e}")

    def run(self, start_date: datetime, end_date: datetime, jump: int, sleep: int):
        current = start_date
        last_cleanup_time = start_date
        ...
        while current < end_date:
            # Check for cleanup (Every 2 days of simulation time)
            if (current - last_cleanup_time) >= timedelta(hours=2):
                # Delete data older than 2 days (rolling window)
                cutoff_dt = current - timedelta(hours=2)
                self.cleanup_cache(int(cutoff_dt.timestamp()))
                last_cleanup_time = current
                 ...
                 for item in data:
                     ...
                     if track:
                        ...
                        if callsign_check and any(callsign_check.startswith(prefix) for prefix in ignored_prefixes):
                             logger.info(f"Skipping {flight_id} (filtered callsign {callsign_check})")
                             continue

                        # Save to cache/rules DB so Proximity rule can find it
