...
def main():
    ...
    if not local_pipeline_available:
        ...
        results = {
            ...
            "layer_5_transformer": {"is_anomaly": False, "score": 0.1, "status": "NORMAL"},
            "layer_6_hybrid": {"is_anomaly": True, "score": 0.88, "status": "ANOMALY"}
        }
    ...
    # Insert Report
    print(f"DB Path: {RESEARCH_DB_PATH.resolve()}")
    
    # Delete first to ensure clean state
    cursor_res.execute("DELETE FROM anomaly_reports WHERE flight_id = ?", (FLIGHT_ID,))
    ...
    print(f"DEBUG: Results keys to be saved: {list(results.keys())}")
    cursor_res.execute("""
